...
public class ZWaveMultiAssociationCommandClass extends ZWaveCommandClass {
	...
	public void handleApplicationCommandRequest(SerialMessage serialMessage, int offset, int endpoint) {
		...
		case MULTI_ASSOCIATIONCMD_REPORT:
			...
			processAssociationReport(serialMessage, offset);
			break;
		case MULTI_ASSOCIATIONCMD_REMOVE:
			logger.trace("Process Association Remove");
			return;
		case MULTI_ASSOCIATIONCMD_GROUPINGSGET:
	...
	protected void processAssociationReport(SerialMessage serialMessage, int offset) {
		...
		logger.debug("NODE {}: association group {} has max associations {}", this.getNode()
				.getNodeId(), group, maxAssociations);
		...
		if (serialMessage.getMessagePayload().length > (offset + 4)) {
			...
			if(dataPointer < dataLength) {
				...
				for (; dataPointer < dataLength; dataPointer += 2) {
					...
					logger.debug("NODE {}: Associated with Node {} endpoint {} in group", this.getNode().getNodeId(), node, endpoint, group);

					// Add the node to the group
					pendingAssociation.addNode(node, endpoint);
				}
			}
		}
	...
	public SerialMessage removeAssociationMessage(int group, int node, int endpoint) {
		logger.debug("NODE {}: Creating new message for command MULTI_ASSOCIATIONCMD_REMOVE", this.getNode()
				.getNodeId());
		SerialMessage result = new SerialMessage(this.getNode().getNodeId(), SerialMessageClass.SendData,
				SerialMessageType.Request, SerialMessageClass.SendData, SerialMessagePriority.Set);
	...
	public SerialMessage getAssociationMessage(int group) {
		logger.debug("NODE {}: Creating new message for command MULTI_ASSOCIATIONCMD_GET group {}", this.getNode()
				.getNodeId(), group);
		SerialMessage result = new SerialMessage(this.getNode().getNodeId(), SerialMessageClass.SendData,
	...
	public SerialMessage getGroupingsMessage() {
		logger.debug("NODE {}: Creating new message for command MULTI_ASSOCIATIONCMD_GROUPINGSGET", this.getNode()
				.getNodeId());
		SerialMessage result = new SerialMessage(this.getNode().getNodeId(), SerialMessageClass.SendData,
				SerialMessageType.Request, SerialMessageClass.ApplicationCommandHandler, SerialMessagePriority.Get);
