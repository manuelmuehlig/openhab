...
public class ZWaveSecurityCommandClass extends ZWaveCommandClass implements ZWaveBasicCommands,
	...
	/**
	 * The code from which this was based included numerous bad security practices (hardcoded IV, voodo to generate
	 * encrypt and auth keys from and existing key).
	 * 
	...
	public void handleApplicationCommandRequest(SerialMessage serialMessage, int offset, int endpoint) {
		...
		if(!verifyAndAdvanceState(command)) {
			return;
		}
	...
	public SerialMessage decryptMessage(byte[] data, int offset) {
		...
		// check for minimum size so we can ignore the return value of bais.read() below
		int minimumSize = offset + ENCAPSULATED_HEADER_LENGTH + AES_BLOCK_SIZE + ENCAPSULATED_FOOTER_LENGTH;
		...
		try {
			...
			if (!secured) {
				// TODO: WTF?!?!?!
				/**
				 * if( ValueBool* value = static_cast<ValueBool*>( GetValue( 1, 0 ) ) ) { value->OnValueRefreshed( true
				 * ); value->Release(); }
				 */
				secured = true;
			}
	...
	private void setupNetworkKey() {
		...
		if (isAddingNode && !networkKeySet) {
			...
			logger.info("NODE {}: Using Configured Network Key addingNode={} keySet={})", this.getNode(), isAddingNode,
					networkKeySet);
			keyBytes = new byte[]{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10}; // TODO: load from saved settings
//			keyBytes = new byte[16]; // TODO: load from saved settings
		}
		debugHex("Network Key", keyBytes);
	 ...
	 * following commans are exchanged:  
	 * 
	 ...
	 * @param newState the state we are about to enter
	 ...
	 * be sent.
	 */
	private synchronized boolean verifyAndAdvanceState(byte newState) {
		if(!EXPECTED_COMMAND_ORDER_LIST.contains(newState)) {
			// Commands absent from EXPECTED_COMMAND_ORDER_LIST are always ok
			return true;
		}
		// Going back to the first step (zero indec) is always OK
		if(EXPECTED_COMMAND_ORDER_LIST.indexOf(newState) > 0) {
			// We have to verify where we are at
			int currentIndex = EXPECTED_COMMAND_ORDER_LIST.indexOf(currentState);
			int newIndex = EXPECTED_COMMAND_ORDER_LIST.indexOf(newState);
	...
	public static void setRealNetworkKey(String hexString) {
		...
		ourString = hexString.replace(" ", "");
		ourString = hexString.replace("0x", "");
		try {
